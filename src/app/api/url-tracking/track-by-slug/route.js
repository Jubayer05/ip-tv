...
export async function POST(request) {
  try {
    ...
    const { visitorId, deviceInfo: clientDeviceInfo, slug } = body;
    ...
    // If pageType is "existing", return the actual page URL (no redirect)
    // If pageType is "non-existing", redirect to homepage
    const redirectUrl =
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        shouldRedirect: urlTracking.pageType !== "existing", // Only redirect if not existing
